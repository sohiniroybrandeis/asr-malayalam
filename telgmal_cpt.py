...
# Load datasets
pt_mal = load_from_disk("cptmal_audio_trans_dataset")
supp_data = load_from_disk("telugu_IS_audio_dataset")

# Set sampling rate for both
sampling_rate = pt_feature_extractor.sampling_rate
pt_mal = pt_mal.cast_column('audio', Audio(sampling_rate=sampling_rate))
...
pt_mal = pt_mal.map(compute_durations, batched=True)
supp_data = supp_data.map(compute_durations, batched=True)

# Step 2: Filter out samples < 3 sec in both
def get_seq_indices_not_too_short(dataset, min_length=3):
    return [i for i, dur in enumerate(dataset["duration"]) if dur > min_length]

good_indices_m = get_seq_indices_not_too_short(pt_mal, 3)
pt_mal = pt_mal.select(good_indices_m)

good_indices_k = get_seq_indices_not_too_short(supp_data, 3)
supp_data = supp_data.select(good_indices_k)

# Step 3: Limit pt_mal to exactly 10 hours
selected_samples = []
total_duration = 0.0
MAX_DURATION_SECONDS = 3600 * 10  # 10 hours

for sample in pt_mal:
    if total_duration + sample["duration"] > MAX_DURATION_SECONDS:
        break
    selected_samples.append(sample)
    total_duration += sample["duration"]

print(f"Selected from cptmal: {total_duration/3600:.2f} hours")
...
train_test_split = combined.train_test_split(test_size=0.05)
...
training_args = TrainingArguments(
	output_dir='pretraining-res-telgmal',
...
repo_name = "cpt6-wav2vec2-large-xls-r-300m-tammal-results"
tokenizer.save_pretrained(repo_name)

feature_extractor = Wav2Vec2FeatureExtractor.from_pretrained("pretraining-res-telgmal/checkpoint-4500")
...
model = Wav2Vec2ForCTC.from_pretrained(
    "pretraining-res-telgmal/checkpoint-4500", 
