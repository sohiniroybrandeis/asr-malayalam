...
import torchaudio
import torch
from transformers import AutoModelForCTC, Wav2Vec2CTCTokenizer, Wav2Vec2BertForCTC, Wav2Vec2FeatureExtractor, Wav2Vec2Processor, Wav2Vec2BertProcessor, Wav2Vec2ForCTC, TrainingArguments, Trainer, SeamlessM4TFeatureExtractor
...
# mal_data["train"] = load_dataset("mozilla-foundation/common_voice_13_0", "ml", split="train+validation")
# mal_data["test"] = load_dataset("mozilla-foundation/common_voice_13_0", "ml", split="test")
...
# vocabs = mal_data.map(extract_all_chars, batched=True, batch_size=-1, keep_in_memory=True, remove_columns=mal_data.column_names["train"])
...
# # tokenizer = Wav2Vec2CTCTokenizer("./results/wav2vec2_malayalam/checkpoint-480/vocab.json", unk_token="[UNK]", pad_token="[PAD]", word_delimiter_token="|")
...
#     args=training_args,
#     compute_metrics=compute_metrics,
#     train_dataset=mal_data["train"],
#     eval_dataset=mal_data["test"],
#     tokenizer=processor.feature_extractor,
# )

trainer.train()

# processor = Wav2Vec2Processor.from_pretrained("./results/wav2vec2_malayalam/checkpoint-480")
# model = Wav2Vec2ForCTC.from_pretrained("./results/wav2vec2_malayalam/checkpoint-480")

model = AutoModelForCTC.from_pretrained("results2")
processor = Wav2Vec2BertProcessor.from_pretrained("results2")
...
# results = mal_data["test"].map(map_to_result, remove_columns=mal_data["test"].column_names)
